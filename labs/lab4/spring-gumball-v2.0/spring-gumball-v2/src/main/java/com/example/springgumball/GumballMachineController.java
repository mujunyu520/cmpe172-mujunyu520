...
public class GumballMachineController {
    ...
    public String postAction(@Valid @ModelAttribute("command") GumballCommand command,
        ...
        // if the past has value is not the same as calculated hash value which is based on the hidden info from the browser,
        // it goes to the error
        if ( !input_hash.equals(calculated_hash) ) {
            throw new GumballServerError() ;
        }

        long ts1 = Long.parseLong( input_timestamp ) ; // the last timestamp
        long ts2 = java.lang.System.currentTimeMillis() ; // current timestamp
        long diff = ts2 - ts1 ;
        ...
        // check the time difference between the last time stamp and the current time stamp
        // Than if the different is greater than 1000 sec, it goes to the error
        if ( (diff/1000) > 1000 ) {
            throw new GumballServerError() ;
        }
